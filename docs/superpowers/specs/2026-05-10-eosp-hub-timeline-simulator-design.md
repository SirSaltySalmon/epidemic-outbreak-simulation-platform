...
- `observation_kind == cohort` and related fields (`cohort_size`, `cohort_deaths`, report windows) are **in scope** for tallies and dashboard truth, but **pinned fixed-agent** semantics apply per **individual** line when `observation_kind == individual`. Implementation plan must specify whether cohorts generate **effective** bursts (e.g. scaled contacts) or are **statistics-only**, without contradicting ingestion rules.
...
- Fallback: scenario flag to use **Poisson** for deterministic debugging (`r → ∞` or separate switch).
...
Replay client provides **toggle** visibility for **A** vs **C**; dashboard default remains **§9.2 C-only**.
